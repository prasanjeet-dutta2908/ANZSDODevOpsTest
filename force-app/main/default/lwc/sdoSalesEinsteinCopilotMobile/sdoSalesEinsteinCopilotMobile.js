...
// Apex Imports
import getGPTConfiguration from "@salesforce/apex/SDO_Sales_GPTController.getMobileGPTConfiguration";
...
export default class SdoSalesEinsteinCopilotMobile extends NavigationMixin(LightningElement) {
    // Builder Properties
    @api recordId;
    ...
    // Data Variables
    responses;
    einsteinGreeting = this.configureMessage("", "", "", "Hi! I’m Einstein, an AI assistant. I can do things like search for information, summarize records, and draft and revise emails. What can I help you with?", "greeting", "slds-message result0", "");
    @track messages = [];
    description;
    @track emailBody;
        ...
        try {
            ...
            switch (action.type) {
                ...
                case "Edit Record":
                    ...
                    break;
                case "Launch Flow":
                    let flowUrl = '/flow/' + action.flowAPIName;
                    if (!action.targetRecordId) {
                        flowUrl += '?recordId=' + this.recordId;
                    } else {
                        flowUrl += '?recordId=' + action.targetRecordId;
                        const flowRef = {
                    ...
                    this[NavigationMixin.Navigate](flowRef);
                    break;
                case "Open URL":
                    const urlRef = {
                        type: 'standard__webPage',
    ...
    gptConfiguration;
    @wire(getGPTConfiguration, {})
    wiredRecordMethod({ error, data }) {
        if (data) {
            console.log(data);
            this.gptConfiguration = data.Id;
            this.config = data;
            this.suggestAction1 = {
